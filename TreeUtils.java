...
public class TreeUtils {
	...
	private boolean isBSTHelper(Node root, int minValue, int maxValue) {
		...
		if( root == null)
			...
			return (root.data <= maxValue && root.data > minValue) &&
	...
	public boolean hasPathSumFromRoot( Node root, int sum) {
		...
		if( root == null)
			...
			return (hasPathSumFromRoot(root.left, sum-root.data) ||
					hasPathSumFromRoot(root.right, sum-root.data));
		}
...
//		}
	...
	public boolean hasPathSum( Node root, int actualSum, int sum ) {
		...
		if( root == null) 
			...
			// reset the sum
			sum = actualSum;
			return (hasPathSum(root.left, actualSum, actualSum-root.data) ||
					hasPathSum(root.right, actualSum, actualSum-root.data));
		}
		else {
			return (hasPathSum(root.left, actualSum, sum-root.data) ||
					hasPathSum(root.right, actualSum, sum-root.data)) ;
		}
	}
